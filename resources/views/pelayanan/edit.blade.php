...
@section('content')
    <div class="flex items-center h-screen  w-full bg-teal-lighter dark:bg-green-500">
        <div class="w-full bg-white rounded shadow-lg p-4 m-32">
            <form class="mb-4" action="{{route('pelayanan.update',$pelayanan->id)}}" method="post">
                @csrf
                <div class="flex flex-col mb-4">
                    <label class="mb-2 uppercase font-bold text-xs text-grey-darkest" for="first_name">Nama Pasien</label>
                    <input class="border py-2 px-3 text-grey-darkest rounded-md" type="text" name="nama_dokter " value="{{$dokter->nama_dokter ?? old('nama_dokter')}}" id="nama_dokter">
